	...
	function testDownload() {
		...
		xhr.onprogress = function(event) {
			if (event.lengthComputable) {
				...
				if (duration > 0.5) { // Wait at least 0.5s for more stable measurement
		...
		function finishDownloadTest() {
			if (testCompleted) return; // Prevent duplicate calls
			testCompleted = true;
			testUpload();
		}
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === XMLHttpRequest.DONE) {
				...
				if (xhr.status === 200 && responseSize > 1000 && duration > 0.2) { // At least 1KB and 0.2s
		...
		if (xhr.upload) {
			xhr.upload.onprogress = function(event) {
				if (event.lengthComputable) {
					var currentTime = Date.now();
					var duration = (currentTime - startTime) / 1000;
					if (duration > 0.3) { // Wait at least 0.3s for stable measurement
