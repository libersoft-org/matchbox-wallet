...
class SystemManager {
	...
	async getSystemBrightness() {
		try {
			...
			const brightnessCommands = [
				...
				'xrandr --verbose | grep -i brightness | head -1 | awk \'{print $2}\' | awk \'{print int($1 * 100)}\'',
				'ddcutil getvcp 10 2>/dev/null | grep -o "current value = [0-9]*" | grep -o "[0-9]*"',
			];
	...
	async setSystemBrightness(params) {
		try {
			...
			for (const cmd of brightnessCommands) {
				try {
					...
					const verifyResult = await this.getSystemBrightness();
