...
if [ "$#" -eq 0 ]; then
 echo "Generating commit message using GitHub Copilot..."
	COMMIT_MSG=$(gh copilot explain "Brief commit message for these changes: `git diff HEAD~1`" 2>/dev/null | awk '/# Explanation:/{flag=1; next} flag && NF > 0' | sed 's/^[[:space:]]*•[[:space:]]*//')
